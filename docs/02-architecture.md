...
Errors are carried inside the plan. A step that could not be built becomes an operator holding the reason, standing where the step would have been and keeping its input, and everything built on top of it is built as usual and surfaces the reason exactly once, at `Collect`. This is what pays for the single error return in the chaining API from document 04. The error names the earliest such step and draws the whole plan with a mark on the operator the mistake is really in, which is why the steps written after the mistake are kept rather than thrown away. Expression building is the one genuinely allocation heavy path in the library, and Go 1.27's cheaper small object allocation helps more here than anywhere else.
